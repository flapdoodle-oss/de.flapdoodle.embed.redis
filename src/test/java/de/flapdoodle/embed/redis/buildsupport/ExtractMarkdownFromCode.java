 ...
 */
package de.flapdoodle.embed.redis.buildsupport;
...
public class ExtractMarkdownFromCode {
	...
	private static String extractMarkdown(String sourceFileName)
			throws IOException {
		List<String> lines = Files.readLines(new File(sourceFileName),
				Charsets.UTF_8);
	...
	private static Collection<String> resolveIncludes(String sourceFileName,
		...
		for (String line : lines) {
			...
			if (includeDefIdx != -1) {
				String fileName = line.substring(
						includeDefIdx + includeTag.length()).trim();
				String basePath = sourceFileName.substring(0,
						sourceFileName.lastIndexOf('/') + 1);
				String includeFilename = basePath + fileName;
				List<String> includedLines = Files.readLines(new File(
						includeFilename), Charsets.UTF_8);
				Collection<String> shiftedLines = Collections2
						.transform(includedLines,
	...
	private static Collection<String> surroundHeadlineWithEmptyLines(
	...
	private static Collection<String> uncommentThreeDots(
			Collection<String> lines) {
		return Collections2.transform(lines,
				new Function<String, String>() {
	...
	private static Collection<String> compactEmptyLines(
			Collection<String> lines) {
		return Collections2.filter(Collections2.transform(lines,
				new Function<String, String>() {
	...
	private static Collection<String> stripNonCodeLines(
			Collection<String> lines) {
		return Collections2.filter(lines, new Predicate<String>() {
			...
			public boolean apply(String input) {
				...
				return (codeStarted && includeCurrentLine)
						| isHeader(input);
			}
		});
	}
	...
	private static Collection<String> startWithFirstHeader(
			Collection<String> lines) {
		Collection<String> withFirstHeader = Collections2.filter(lines,
				new Predicate<String>() {
